...
def extrair_unidades(desc, palavras_chave):
    if not isinstance(desc, str):
        return None
    desc = desc.lower()
    desc = re.sub(r'\b\d+(\.\d+)?\s*(g|gr|grm|kg|ml|l|lt|litros?)\b', '', desc)
...
def compara_contenido_global(row):
    try:
        return "OK" if int(row.get("#BR LOC 008 : CONTENIDO G2G")) == int(row.get("GLOBAL TOTAL ITEMS IN PACK")) else "DIVERGENTE"
    except:
...
def comparar_analise_vs_global_pack(row):
    try:
        ...
        g = int(re.search(r'\d+', str(row["GLOBAL TOTAL PACKS IN MULTIPACK"])).group())
